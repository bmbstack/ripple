...
func Generate(currentPath, component, name string) {
	...
	if strings.EqualFold(component, "all") {
		...
		logger.Logger.Debug(fmt.Sprintf("[%s]generate code rpcClient.service.go ...", component))
...
%s`, pkg, pbPkg, codeVar, codeFunc)
	...
	for _, item := range funcList {
		...
		hasFunc := rst.HasFuncDeclInFile(df, dst.FuncDecl{Name: &dst.Ident{Name: fmt.Sprintf("Get%s", itemValue)}})
		if !hasFunc {
							...
							Body: &dst.BlockStmt{
								...
								List: []dst.Stmt{
									&dst.AssignStmt{
										...
										Tok: token.ASSIGN,
										Rhs: []dst.Expr{&dst.CallExpr{Fun: &dst.Ident{Name: fmt.Sprintf("New%s", upper), Path: pbPkg}}},
									},
								},
							},
						},
