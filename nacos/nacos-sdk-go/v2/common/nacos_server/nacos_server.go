...
import (
	...
	security2 "github.com/bmbstack/ripple/nacos/nacos-sdk-go/v2/common/security"
...
type NacosServer struct {
	sync.RWMutex
	securityLogin         security2.AuthClient
	serverList            []constant2.ServerConfig
	httpAgent             http_agent2.IHttpAgent
	timeoutMs             uint64
	endpoint              string
...
func NewNacosServer(serverList []constant2.ServerConfig, clientCfg constant2.ClientConfig, httpAgent http_agent2.IHttpAgent, timeoutMs uint64, endpoint string) (*NacosServer, error) {
	...
	securityLogin := security2.NewAuthClient(clientCfg, serverList, httpAgent)
...
func (server *NacosServer) callConfigServer(api string, params map[string]string, newHeaders map[string]string,
	...
	start := time.Now()
	if contextPath == "" {
		contextPath = constant2.WEB_CONTEXT
	}
	...
	headers["Client-Version"] = []string{constant2.CLIENT_VERSION}
	headers["User-Agent"] = []string{constant2.CLIENT_VERSION}
	...
	monitor2.GetConfigRequestMonitor(method, url, util2.GetStatusCode(response)).Observe(float64(time.Now().Nanosecond() - start.Nanosecond()))
	if err != nil {
		return
	}
	var bytes []byte
	...
	if response.StatusCode == constant2.RESPONSE_CODE_SUCCESS {
		...
		err = nacos_error2.NewNacosError(strconv.Itoa(response.StatusCode), string(bytes), nil)
		return
	}
}
...
func (server *NacosServer) callServer(api string, params map[string]string, method string, curServer string, contextPath string) (result string, err error) {
	...
	result = string(bytes)
	monitor2.GetNamingRequestMonitor(method, api, util2.GetStatusCode(response)).Observe(float64(time.Now().Nanosecond() - start.Nanosecond()))
	if response.StatusCode == constant2.RESPONSE_CODE_SUCCESS {
		return
	} else {
...
func (server *NacosServer) ReqConfigApi(api string, params map[string]string, headers map[string]string, method string, timeoutMS uint64) (string, error) {
	...
	var result string
	if len(srvs) == 1 {
		for i := 0; i < constant2.REQUEST_DOMAIN_RETRY_TIME; i++ {
			result, err = server.callConfigServer(api, params, headers, method, getAddress(srvs[0]), srvs[0].ContextPath, timeoutMS)
			if err == nil {
				return result, nil
			}
			logger2.Errorf("api<%s>,method:<%s>, params:<%s>, call domain error:<%+v> , result:<%s>", api, method, util2.ToJsonString(params), err, result)
		}
		return "", err
	} else {
		...
		for i := 1; i <= len(srvs); i++ {
			...
			logger2.Errorf("[ERROR] api<%s>,method:<%s>, params:<%s>, call domain error:<%+v> , result:<%s> \n", api, method, util2.ToJsonString(params), err, result)
...
func (server *NacosServer) ReqApi(api string, params map[string]string, method string) (string, error) {
	...
	if len(srvs) == 1 {
		...
		return "", errors.New("retry " + strconv.Itoa(constant2.REQUEST_DOMAIN_RETRY_TIME) + " times request failed!")
	} else {
		index := rand.Intn(len(srvs))
		for i := 1; i <= len(srvs); i++ {
			curServer := srvs[index]
...
func (server *NacosServer) refreshServerSrvIfNeed() {
	if util2.CurrentMillis()-server.lastSrvRefTime < server.vipSrvRefInterMills && len(server.serverList) > 0 {
		return
	}
	...
	logger2.Infof("http nacos server list: <%s>", result)

	var servers []constant2.ServerConfig
	...
	for _, line := range list {
		if line != "" {
			...
			if len(splitLine) == 2 {
				...
				if err != nil {
					logger2.Errorf("get port from server:<%s>  error: <%+v>", line, err)
					continue
				}
			}

			servers = append(servers, constant2.ServerConfig{Scheme: constant2.DEFAULT_SERVER_SCHEME, IpAddr: splitLine[0], Port: uint64(port), ContextPath: contextPath})
		}
	}
	if len(servers) > 0 {
		if !reflect.DeepEqual(server.serverList, servers) {
			server.Lock()
			logger2.Infof("server list is updated, old: <%v>,new:<%v>", server.serverList, servers)
			server.serverList = servers
			server.ServerSrcChangeSignal <- struct{}{}
			server.lastSrvRefTime = util2.CurrentMillis()
			server.Unlock()
		}
...
func (server *NacosServer) InjectSecurityInfo(param map[string]string) {
	...
	if accessToken != "" {
		param[constant2.KEY_ACCESS_TOKEN] = accessToken
	}
}

func getAddress(cfg constant2.ServerConfig) string {
...
func GetSignHeaders(params map[string]string, secretKey string) map[string]string {
	...
	timeStamp := strconv.FormatInt(util2.CurrentMillis(), 10)
...
func (server *NacosServer) GetNextServer() (constant2.ServerConfig, error) {
	...
	if serverLen == 0 {
		return constant2.ServerConfig{}, errors.New("server is empty")
	}
...
func (server *NacosServer) InjectSkAk(params map[string]string, clientConfig constant2.ClientConfig) {
	if clientConfig.AccessKey != "" {
		params[constant2.KEY_ACCESS_KEY] = clientConfig.AccessKey
	}
	if clientConfig.SecretKey != "" {
		params[constant2.KEY_SECRET_KEY] = clientConfig.SecretKey
	}
}
