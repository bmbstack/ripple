...
func newXClientPoolForUser() (*client.XClientPool, error) {
	...
	pool := client.NewXClientPool(poolSize, ServiceNameOfUser, failMode, selectMode, d, opt)
...
func NewUserClient() *UserClient {
	pool, err := newXClientPoolForUser()
	if err != nil {
		fmt.Println(fmt.Sprintf("Create rpcx client err: orderserver", err.Error()))
		return &UserClient{}
	}
	return &UserClient{XClientPool: pool, Plugins: client.NewPluginContainer()}
}

// NewUserClientWithPlugins wraps a XClient as UserClient.
// You can pass a shared XClient object created by NewXClientForUser.
func NewUserClientWithPlugins(plugins client.PluginContainer) *UserClient {
	pool, err := newXClientPoolForUser()
	if err != nil {
		fmt.Println(fmt.Sprintf("Create rpcx client err: orderserver", err.Error()))
		return &UserClient{}
	}
	return &UserClient{XClientPool: pool, Plugins: plugins}
}
